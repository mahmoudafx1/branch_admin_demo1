...
import { LayoutDashboard, CheckSquare, DollarSign, LogOut, Store, Users, Box, CreditCard, Megaphone, LayoutGrid, CalendarCheck } from "lucide-react";
    ...
    return (
        ...
        >
            ...
            <div className={`flex items-center mb-8 h-[35px] transition-all duration-400 ${isOpen ? "px-2 gap-3" : "justify-center px-0"}`}>
                ...
                {isOpen && (
                    <span className="text-white text-xl font-bold whitespace-nowrap animate-in fade-in duration-300">
                        BooklyX
                    </span>
                )}
            </div>
            ...
            <ul className="flex flex-col gap-3 flex-grow px-3">
                ...
                <SidebarItem icon={<Users size={22} />} text="Staff" isOpen={isOpen} to="/staff" />
                <SidebarItem icon={<Box size={22} />} text="Services" isOpen={isOpen} to="/users" />
                <SidebarItem
                    icon={<CreditCard size={22} />}
                    text="Subscriptions"
                    isOpen={isOpen}
                    to="/subscriptions"
                    isActiveOverride={location.pathname === '/subscriptions' || location.pathname.startsWith('/subscriptions/')}
                />
                <SidebarItem icon={<Megaphone size={22} />} text="Offers" isOpen={isOpen} to="/offers" />
                <SidebarItem icon={<DollarSign size={22} />} text="Finance" isOpen={isOpen} to="/finance" />
                <SidebarItem icon={<CalendarCheck size={22} />} text="Booking" isOpen={isOpen} to="/booking" />
            </ul>
                ...
                <div className={`flex items-center transition-all duration-400 ${isOpen ? "gap-3" : "justify-center"}`}>
                    ...
                    {isOpen && (
                        <div className="overflow-hidden whitespace-nowrap animate-in slide-in-from-left-2">
                            <p className="text-white text-[13px] font-bold">Branch Admin</p>
